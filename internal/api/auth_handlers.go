...
// the main Resolvarr UI for visual consistency.
...
<head>
<meta charset="utf-8">
<title>Resolvarr — First-run setup</title>
...
<div class="card">
  ...
  <footer>Resolvarr {{.Version}} · security · <a href="https://github.com/prophetse7en/resolvarr" style="color:#4a90e2;text-decoration:none">github</a></footer>
</div>
</body>
</html>`))
...
func (s *Server) handleSetupPage(w http.ResponseWriter, r *http.Request) {
	...
	if err := setupTmpl.Execute(w, map[string]any{
		"Version":   s.Version,
		"CSRFToken": s.AuthStore.EnsureCSRFCookie(w, r),
	}); err != nil {
...
func (s *Server) handleSetupSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseForm(); err != nil {
		s.renderSetupError(w, r, "", "Invalid form data")
		return
	}
	...
	if password != confirm {
		s.renderSetupError(w, r, username, "Passwords do not match")
		return
	}
	if err := s.AuthStore.Setup(username, password); err != nil {
		s.renderSetupError(w, r, username, err.Error())
		return
	}

	// Auto-login: create session so user lands directly in the UI.
	sessionID, err := s.AuthStore.CreateSession()
	if err != nil {
...
func (s *Server) renderSetupError(w http.ResponseWriter, r *http.Request, username, errMsg string) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Header().Set("Cache-Control", "no-store")
...
<div class="card">
  ...
  <footer>Resolvarr {{.Version}}</footer>
</div>
</body>
</html>`))
...
func (s *Server) handleLoginSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if !s.AuthStore.VerifyPassword(username, password) {
		// Generic error — do not distinguish "wrong user" from "wrong pw".
		s.renderLoginError(w, r, username, "Invalid username or password")
		return
	}
...
func (s *Server) renderLoginError(w http.ResponseWriter, r *http.Request, username, errMsg string) {
	next := sanitizeNext(r.URL.Query().Get("next"))
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Header().Set("Cache-Control", "no-store")
...
func (s *Server) handleAuthStatus(w http.ResponseWriter, r *http.Request) {
	...
	if s.AuthStore.IsConfigured() {
		if cookie, err := r.Cookie(auth.SessionCookieName); err == nil {
			if s.AuthStore.ValidateSession(cookie.Value) {
				authenticated = true
			}
		}
	}
	...
	// fails later.
	adminEquivalent := authenticated || s.isLocalBypassRequest(r) || s.AuthStore.Config().Mode == auth.ModeNone

	cfg := s.AuthStore.Config()
	resp := map[string]any{
		"configured":              s.AuthStore.IsConfigured(),
	...
	if adminEquivalent {
		resp["username"] = s.AuthStore.Username()
		resp["trusted_networks_locked"] = s.AuthStore.TrustedNetworksLocked()
		resp["trusted_proxies_locked"] = s.AuthStore.TrustedProxiesLocked()
		if s.AuthStore.TrustedNetworksLocked() {
			resp["trusted_networks_effective"] = s.AuthStore.TrustedNetworksRaw()
		}
		if s.AuthStore.TrustedProxiesLocked() {
			resp["trusted_proxies_effective"] = s.AuthStore.TrustedProxiesRaw()
		}
	}
...
func (s *Server) isLocalBypassRequest(r *http.Request) bool {
	cfg := s.AuthStore.Config()
	...
	return s.AuthStore.IsLocalAddress(clientIP)
}
...
func (s *Server) handleGetAPIKey(w http.ResponseWriter, r *http.Request) {
	...
	key := s.AuthStore.APIKey()
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Cache-Control", "no-store")
	json.NewEncoder(w).Encode(map[string]string{"api_key": key})
}

func (s *Server) handleRegenAPIKey(w http.ResponseWriter, r *http.Request) {
	...
	newKey, err := s.AuthStore.RegenerateAPIKey()
	if err != nil {
...
func (s *Server) handleChangePassword(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.AuthStore.ChangePassword(req.CurrentPassword, req.NewPassword); err != nil {
	...
	newSID, err := s.AuthStore.CreateSession()
	if err != nil {
...
func (s *Server) setSessionCookie(w http.ResponseWriter, r *http.Request, sessionID string) {
	secure := r.TLS != nil
	if !secure && s.AuthStore.IsRequestFromTrustedProxy(r) {
		secure = strings.EqualFold(r.Header.Get("X-Forwarded-Proto"), "https")
	}
	ttl := s.AuthStore.Config().SessionTTL
	http.SetCookie(w, &http.Cookie{
		Name:     auth.SessionCookieName,
		Value:    sessionID,
		Path:     "/",
