...
func (pushoverProvider) Validate(agent Agent) error {
	...
	if isLiteralPlaceholder(agent.Config.PushoverUserKey) || isLiteralPlaceholder(agent.Config.PushoverAppToken) {
		return fmt.Errorf("pushover credentials are the masked placeholder — re-enter the real user key + app token")
	}
	return nil
}
