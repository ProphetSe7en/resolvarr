...
const (
	...
	// Exported so the core config-load can detect persisted corruption
	// (pre-deep-copy-fix configs may have stored this verbatim).
...
// isLiteralPlaceholder returns true when the persisted/incoming value is
// the literal mask string. This shouldn't happen in normal flow — masking
// only ever runs on the response side, and preserveIfMasked restores the
// real value before storage. But a pre-existing corruption (from before
// the ConfigStore.Get deep-copy fix in commit 26b76fd) can leave an agent
// stored with the literal placeholder. We detect and reject in Validate
// so subsequent saves can't keep the corruption alive, and so a fresh
// Discord/Gotify/etc fire surfaces a clear error instead of an opaque
// "is not snowflake" 400 from the upstream API.
func isLiteralPlaceholder(value string) bool {
	v := strings.TrimSpace(value)
	return v == maskedDiscordWebhook || v == maskedToken
}
