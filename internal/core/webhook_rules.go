...
func WebhookFunctionAppliesTo(fn WebhookFunction, appType string) bool {
	...
	case WebhookFnTagAudio, WebhookFnTagVideo, WebhookFnRecover, WebhookFnFileDeleteClean, WebhookFnGrabRename:
...
func EventsForFunction(fn WebhookFunction, appType string) []WebhookConnectEvent {
	...
	case WebhookFnTagReleaseGroups, WebhookFnDiscover, WebhookFnTagAudio, WebhookFnTagVideo,
		WebhookFnTagDvDetail, WebhookFnRecover, WebhookFnSyncToSecondary:
...
// QbitSeRules holds the Sonarr-only qBit episode/season tag toggles.
// Both flags can be on simultaneously — typical setup is "tag with
// season AND episode so the user can filter qBit by either".
type QbitSeRules struct {
	TagSeason  bool `json:"tagSeason,omitempty"`  // S01 / S02 …
	TagEpisode bool `json:"tagEpisode,omitempty"` // S01E05 / S01E05E06 (multi-ep)
...
type WebhookRule struct {
	...
	// GrabRename + QbitSe are the per-function detail configs. Pointer
	// types so the JSON omits them when the function isn't enabled.
