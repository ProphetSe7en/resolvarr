...
function app() {
  return {
    ...
    // ---- Release Type Overview (Sonarr-only, read-only) ----
    ...
    async confirmReplay() {
      ...
      try {
        ...
        // Refresh the instance's events so the fresh re-run entry + its
        // outcome appear (the SSE stream usually pushes it live too).
    ...
    scanSectionVisible(section) {
      ...
      // release types only exist on Sonarr episode files, movies are
      // always single files.)
    ...
    // ---- Profile by tag (one-off Library scan; modal wizard + result modal) ----
